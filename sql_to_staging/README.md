...
(Als je deze module wil uitproberen met synthetische data en zonder productie-database, zie de demo-scripts in `sql_to_staging/demo/`.)

## Configuratie

`sql_to_staging` gebruikt één INI-bestand (`--config`) met de secties
- `[database-source]`, `[database-destination]`, 
, `[settings]` en optioneel `[logging]`.
Een voorbeeld staat in `sql_to_staging/config.ini.example`. 
Je kan ook via environment-variables configureren; zie voorbeeld
`sql_to_staging/.env.example`. 
...
(`sql_to_staging/config.ini.example` en `sql_to_staging/.env.example`).
