...
def test_main_source_to_staging_sqlite(tmp_path):
    ...
    cfg_path = tmp_path / "source_to_staging.ini"
    cfg_path.write_text(
        f"""
[database-source]
SRC_DRIVER=sqlite
...
""".strip()
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "source_to_staging.main", "--config", str(cfg_path)],
        cwd=str(Path.cwd()),
        env=env,
        capture_output=True,
        text=True,
    ...
    if proc.returncode != 0:
        raise AssertionError(
            f"source_to_staging.main failed: {proc.returncode}\nSTDOUT:\n{proc.stdout}\nSTDERR:\n{proc.stderr}"
        )
