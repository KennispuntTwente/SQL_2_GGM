...
def test_mssql_strict_lower_pref_against_uppercase_source(tmp_path):
    # Ensure clean env for staging name/column matching
    for k in [
    ...
    # Start SQL Server and create target table and uppercase-named staging table
    port = 1438
    engine = get_connection(
        db_type="mssql",
        db_name="src_case_mixed",
        user="sa",
    ...
    with engine.begin() as conn:
        ...
        # Upper-case source table name with UPPER columns
        ...
        conn.execute(text("INSERT INTO dbo.WVBESL (BESLUITNR, CLIENTNR) VALUES ('B901','C901');"))
    ...
    # Helper to insert from select while preserving select column order
    ...
    # Strict mode + lower column name preference against UPPER source columns
    ...
    with engine.connect() as conn:
        cnt = conn.execute(text("SELECT COUNT(*) FROM dbo.BESCHIKKING WHERE CLIENT_ID='C901'")) .scalar_one()
    assert cnt == 1
