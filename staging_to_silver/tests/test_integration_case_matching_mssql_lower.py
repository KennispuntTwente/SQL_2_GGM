...
def test_mssql_case_matching_prefers_lowercase_source(tmp_path):
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text("INSERT INTO dbo.wvbesl (besluitnr, clientnr) VALUES ('B300','c300');")
        )
    ...
    with engine.connect() as conn:
        cnt = conn.execute(
            text("SELECT COUNT(*) FROM dbo.BESCHIKKING WHERE CLIENT_ID='c300'")
        ).scalar_one()
    assert cnt == 1
