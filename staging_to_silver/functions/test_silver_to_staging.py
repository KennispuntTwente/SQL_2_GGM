...
def fill_engine_with_data(engine, schema, date_mode="epoch"):
    ...
    date_mode:
      ...
      - "epoch_ns"     -> dd_begin/dd_eind are BigInteger epoch nanoseconds
    ...
    if date_mode in {"epoch", "epoch_ms", "epoch_us", "epoch_ns"}:
        dd_type = BigInteger
    ...
    elif date_mode == "epoch_ns":
        row_wvind_b = {
            "besluitnr": "B001",
            "volgnr_ind": "01",
            "dd_eind": 1704067200 * 1_000_000_000,  # ns
            "dd_begin": 1701475200 * 1_000_000_000,  # ns
            "volume": 10,
            "status_indicatie": "active",
            "kode_regeling": "KR1",
            "eenheid": "UUR",
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            insert(szregel), [{"kode_regeling": "KR1", "omschryving": "JEUGDWET"}]
        )
        ...
        conn.execute(
            ...
            [
                {
                    "code": "RC1",
                    "omschrijving": "Some reason",
                    "domein": "JZG_REDEN_EINDE_PRODUCT",
                }
            ],
        )

    print(
        f"Dummy staging data inserted into schema '{schema}' with date_mode='{date_mode}'."
    )
