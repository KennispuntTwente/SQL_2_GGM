from sqlalchemy import MetaData, select, func, literal


def build_rechtspersoon(engine, source_schema=None):
    ...
    wvaanb, szcred = (metadata.tables[name] for name in fq_names)

    adres_expr = func.concat(
    ...
    return (
        select(
            ...
            adres_expr.label("ADRESBINNENLAND"),
            adres_expr.label("ADRESCORRESPONDENTIE"),
        )
        .select_from(wvaanb)
...
__query_exports__ = {
    "RECHTSPERSOON": build_rechtspersoon,
}
